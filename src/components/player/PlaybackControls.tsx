    ...
    return (
        <div className="relative z-10 px-12 pb-12">
            ...
            <div className="flex items-center gap-4 mb-6 group">
                ...
                <div className="flex-1 h-1.5 bg-[var(--glass-border)] rounded-full overflow-hidden cursor-pointer relative" onClick={onSeek}>
                    <div className="h-full bg-[var(--text-main)] rounded-full group-hover:bg-[var(--accent-color)] relative transition-all duration-300 ease-linear" style={{ width: `${progressPercent}%` }}>
                        <div className="absolute right-0 top-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 bg-[var(--text-main)] rounded-full shadow opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    </div>
                </div>
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-8">
                    <button onClick={onToggleMode} className={`btn-icon w-10 h-10 transition-colors ${shuffle || repeat === 'one' ? 'text-[var(--accent-color)] active:scale-95' : 'hover:text-[var(--text-main)]'}`}>
                    ...
                    <button onClick={onToggleQueue} className={`btn-icon w-10 h-10 transition-colors ${showQueue ? 'text-[var(--accent-color)]' : 'hover:text-[var(--text-main)]'}`}>
                ...
                <div className="flex gap-4 items-center justify-end w-[120px]">
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[var(--glass-border)] border border-[var(--glass-border)] whitespace-nowrap">
                        <span className="text-[10px] text-[var(--text-muted)] border-r border-[var(--glass-border)] pr-2 mr-0.5">{currentTrack.source}</span>
