...
import { ListMusic, Trash2, Target, Check, X } from 'lucide-react';
...
const QueueTrackItem = React.memo<QueueTrackItemProps>(({
    ...
    return (
        <div
            ...
            className={`flex items-center gap-4 py-3 pl-8 pr-6 cursor-pointer transition-all group relative ${isCurrent ? 'bg-[var(--accent-color)]/10 dark:bg-[var(--accent-color)]/20' : 'hover:bg-[var(--glass-highlight)]'}`}
        >
                ...
                <div className={`absolute inset-0 flex items-center justify-center ${isCurrent && isPlaying ? 'bg-black/40' : 'bg-black/20 opacity-0 group-hover:opacity-100'} transition-opacity`}>
                    {isCurrent && isPlaying ? (
                        <div className="flex gap-0.5 items-end h-4">
                            ...
                            <div className="w-0.5 bg-white animate-[music-bar_0.8s_ease-in-out_infinite] h-2/3" />
                            <div className="w-0.5 bg-white animate-[music-bar_0.7s_ease-in-out_infinite] h-5/6" />
                        </div>
                    ) : (
