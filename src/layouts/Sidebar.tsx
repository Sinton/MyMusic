...
const Sidebar: React.FC<SidebarProps> = ({ activeView, onNavigate, onOpenAuth }) => {
    ...
    return (
        <div className="glass-panel h-full flex flex-col" style={{ width: 'var(--sidebar-width)', flexShrink: 0 }}>
            ...
            {/* Platform Sections - Scrollable Center */}
                ...
                <div className="space-y-1.5">
                    {sections.map((section) => {
                        ...
                        const renderPlaylistItem = (pl: Playlist, isNested = false) => {
                            ...
                            return (
                                <button
                                    ...
                                    title={pl.title}
                                    className={`w-full flex items-center gap-3 pr-3 py-1.5 rounded-lg transition-colors group ${isNested ? 'pl-10 text-[11px]' : 'pl-8 text-xs'} ${isSelected
                                        ? 'bg-[var(--glass-border)] text-[var(--text-main)] font-medium'
                                        : 'text-[var(--text-secondary)] hover:text-[var(--text-main)] hover:bg-[var(--glass-highlight)]'
                                        }`}
                                >
                        ...
                        const renderSubHeader = (groupId: string, label: string) => (
                            ...
                            >
                                <ChevronRight className={`w-3 h-3 transition-transform ${isGroupExpanded(`${section.id}_${groupId}`) ? 'rotate-90' : ''}`} />
                                {label}
                            </button>
                        );
                        ...
                        return (
                            <div key={section.id} className="space-y-0.5">
                                ...
                                <div className="flex items-center">
                                    ...
                                    >
                                        <ChevronRight className={`w-3.5 h-3.5 transition-transform ${isSectionExpanded ? 'rotate-90' : ''}`} />
